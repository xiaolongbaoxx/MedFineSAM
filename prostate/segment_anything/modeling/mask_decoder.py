...
class MaskDecoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.decoder_heat = Heat2D(dim=transformer_dim, hidden_dim=transformer_dim)
        self.decoder_freq_mlp = nn.Sequential(
                       nn.Linear(transformer_dim, 1024),
                       nn.ReLU(),
                       nn.Linear(1024, transformer_dim * 14 * 14),
            )

        self.decoder_scale = nn.Parameter(torch.tensor(0.5))
    ...
    def predict_masks(
        ...
        b, c, h, w = src.shape
        proto_dec = src.mean(dim=[2, 3])  # [B, C]
        freq = self.decoder_freq_mlp(proto_dec).view(b, 14, 14, c)
        freq = freq.permute(0, 3, 1, 2)  # [B, C, 14, 14]
        freq = F.interpolate(freq, size=(h, w),
                                                                   mode = 'bicubic', align_corners = False)
        freq = freq.permute(0, 2, 3, 1).contiguous()  # [B, H, W, C]
        heat_dec = self.decoder_heat(src, freq)  # [B, C, H, W]
        src = src + self.decoder_scale * heat_dec
