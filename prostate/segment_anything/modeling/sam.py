...
from prostate.segment_anything.modeling.modules.channel import ChannelGate
...
class Sam(nn.Module):
    ...
    def forward_train(self, batched_input, multimask_output, image_size):
        ...
        low_res_masks, iou_predictions = self.mask_decoder(
            ...
            dense_prompt_embeddings=dense2,
            confidence_score=iou2,
            multimask_output=multimask_output
        )
    ...
    def _make_freq_embed(self, prototype, H, W):
        B, _ = prototype.shape
        freq = self.freq_mlp(prototype).view(B, 14, 14, -1)  # [B,14,14,C]
        freq = freq.permute(0, 3, 1, 2)
        freq = F.interpolate(freq, size=(H, W), mode='bicubic', align_corners=False)
        return freq.permute(0, 2, 3, 1).contiguous()  # [B,H,W,C]
    ...
    def forward_test(
        ...
        freq_embed = freq_embed.permute(0, 3, 1, 2)
        freq_embed = F.interpolate(freq_embed, size=(H, W), mode='bicubic', align_corners=False)
        freq_embed = freq_embed.permute(0, 2, 3, 1).contiguous()
        ...
        for image_record, curr_embedding in zip(batched_input, image_embeddings):
            if "point_coords" in image_record:
                ...
                points = None
            sparse_embeddings, dense_embeddings, confidence_score = self.prompt_encoder(
                points=points,
    ...
    def preprocess(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Normalize colors
        x = (x - self.pixel_mean) / self.pixel_std   # 期望 x: [B, 3, H, W]
