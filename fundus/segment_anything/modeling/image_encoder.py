...
class ImageEncoderViT(nn.Module):
    ...
    def forward(self, x: torch.Tensor) :
        ...
        for i, blk in enumerate(self.blocks):
            ...
            x = blk(x, embedding_feature, layer_id=i)#
...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor, embedding_feature, layer_id=None) -> torch.Tensor:#
        shortcut = x
        x = self.norm1(x)
        # Window partition
        ...
        x = shortcut + x
        xn = self.norm2(x)
        x = x + self.mlp(xn) + self.scale * self.channel_adapter(xn, embedding_feature,layer_id=layer_id)#
