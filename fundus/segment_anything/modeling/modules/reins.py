...
#         # 🛠️ 最终防呆处理：无论 tokens 是 [1, M, C] 或 [1, M, 1, C] 或 [M, C]，都统一处理成 [M, C]
#         while tokens.dim() > 2:
...
class Reins(nn.Module):
    def __init__(
        ...
        self.register_buffer("_usage_sum", torch.zeros(self.num_layers, self.token_length), persistent=False)  # 加权口径
        self.register_buffer("_usage_top1", torch.zeros(self.num_layers, self.token_length), persistent=False)  # top1 口径
        self._eps = 1e-12
        # __init__ 末尾
    ...
    def _tally_usage(self, layer_idx: int, attn: torch.Tensor):
        """
        attn: [N, B, M] 这里与你 forward_delta_feat 中的 attn 对齐
        统计两种口径：
          1) sum：把每张图、每个位置对每个 token 的注意力概率累加（更平滑、可比较）
          2) top1：把每个位置“最像的那个 token”计一次数（更离散、可解释）
        """
        # 统一计算在 softmax 之后的 attn（含 token0=静默 token）
        # ① 加权口径（含 token0）
        sum_over_NB = attn.sum(dim=(0, 1))  # [M]
        self._usage_sum[layer_idx, :].add_(sum_over_NB)

        # ② top1 口径（含 token0），对每个 (n,b) 取 argmax
    ...
    def forward_delta_feat(self, feats: Tensor, tokens: Tensor, layers: int) -> Tensor:
        ...
        # === 统计（务必放在 softmax 之后）===
        if self.log_usage:
            # 1) 兼容两种维度顺序：希望是 [N,B,M]；若是 [B,N,M] 就转置一下
            if attn.dim() != 3:
                pass  # 形状异常直接跳过
            else:
